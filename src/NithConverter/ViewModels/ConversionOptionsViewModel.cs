...
public sealed class ConversionOptionsViewModel : ObservableObject
{
    ...
    public ConversionOptionsViewModel()
    {
        ...
        _audioBitrate = AudioBitrateChoices[2]; _documentDpi = DocumentDpiChoices[1];
    }
    ...
    public Visibility ImageQualityVisibility => _kind != MediaKind.Video && _format is "JPG" or "WEBP" ? Visibility.Visible : Visibility.Collapsed;
    public Visibility PngVisibility => _format == "PNG" ? Visibility.Visible : Visibility.Collapsed;
    public Visibility VideoQualityVisibility => _kind == MediaKind.Video && _format != "GIF" ? Visibility.Visible : Visibility.Collapsed;
    public Visibility VideoVisibility => _kind == MediaKind.Video ? Visibility.Visible : Visibility.Collapsed;
    public Visibility AudioVisibility => _kind == MediaKind.Video && _format != "GIF" ? Visibility.Visible : Visibility.Collapsed;
    ...
    public string ContextLabel => _kind == MediaKind.Video ? "VÍDEO" : _kind == MediaKind.Document ? "DOCUMENTO" : _kind == MediaKind.Image ? "IMAGEM" : "IMAGENS · PDF · VÍDEOS";
    public string Hint => _kind == MediaKind.Video
        ? _format == "GIF"
    ...
    public void Configure(MediaKind? kind, string? format)
    {
        ...
        {
            nameof(AvailableVisibility), nameof(ImageQualityVisibility), nameof(PngVisibility), nameof(VideoQualityVisibility),
            nameof(VideoVisibility), nameof(AudioVisibility), nameof(DocumentVisibility), nameof(GifVisibility), nameof(ContextLabel), nameof(Hint)
        }) Raise(name);
    }
    public ConversionOptions Snapshot() => new()
    {
        ...
        AudioBitrateKbps = _audioBitrate.Value, DocumentDpi = _documentDpi.Value
    };
