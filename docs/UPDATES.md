...
3. Compara a tag da release (`v1.1.1`, por exemplo) com a versão do executável instalado.
...
git init
git add .
git commit -m "NITH Converter 1.1.0"
git branch -M main
...
O código desta entrega está com versão base `1.1.0`. Depois que `main` estiver no GitHub:
...
```powershell
git add .
git commit -m "Atualização 1.1.1"
git push origin main
git tag v1.1.1
git push origin v1.1.1
```

Quem estiver usando `1.1.0` verá `1.1.1`, o instalador será baixado automaticamente (configuração padrão) e ficará pronto para instalação.
...
1. Publique e instale `v1.1.0`.
2. Faça uma pequena mudança no código.
3. Publique a tag `v1.1.1`.
4. Abra a instalação `1.1.0`.
5. O app deve encontrar `1.1.1` e baixar o instalador.
